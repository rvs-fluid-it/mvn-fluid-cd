...
import be.fluid_it.mvn.cd.x.facets.PolishException;
import be.fluid_it.mvn.cd.x.facets.resolve.FacetResolver;
import be.fluid_it.mvn.cd.x.facets.model.MavenConventions;
...
public class DefaultPomPolisher implements PomPolisher {
    ...
    public File polish(File pomFile) {
        ...
        try {
            File polishedPomFile = new File(pomFile.getParentFile(), MavenConventions.POLISHED_POM_FILE);
            polish(new FileInputStream(pomFile), new FileOutputStream(polishedPomFile));
            logger.info("[PomPolisher]: " +
                    pomFile.getName() +
                    " is polished as " + MavenConventions.POLISHED_POM_FILE);
